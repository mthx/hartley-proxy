...
describe('proxy', () => {
  ...
  it('proxies a simple request specified via the path', async () => {
    const { response, body } = await proxyRequest(proxy, {
      path: backend.url().href,
    });
  ...
  it('proxies a request with a body', async () => {
    ...
    const { response, body } = await proxyRequest(
      proxy,
      { method: 'PUT', headers: { host: backend.url().host }, path: '/' },
      requestBody
    );
